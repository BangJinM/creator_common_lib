...
export class LoaderFactory {
    static GetResoureLoader(resource: IResource): IResourceLoader {
        ...
        if (resource.bundleCache) { loader = new BundleLoader(resource) }
        else if (resource.assetType == cc.SpriteFrame) { loader = new SpriteFrameLoader(resource) }
        ...
        else if (resource.assetType == cc.SceneAsset) { loader = new SceneLoader(resource) }
